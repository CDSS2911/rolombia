            ...
            if ( other ) then
                if ( player ~= other ) then
                    ...
                    if ( getDistanceBetweenPoints3D ( x, y, z, getElementPosition ( other ) ) < 5 ) then
						...
						return end
						local canCarry, currentWeight, newWeight, maxWeight = exports.items:canCarryItem( other, 29, tostring(weapon), "Arma " .. tostring(weapon), ammo )
						if not canCarry then
							outputChatBox("El otro jugador no puede cargar mas peso. Peso: " .. string.format("%.1f", currentWeight) .. "/" .. string.format("%.1f", maxWeight) .. " kg. Arma: +" .. string.format("%.1f", newWeight) .. " kg.", player, 255, 0, 0)
							return
						end
