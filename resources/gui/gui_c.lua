...
local max_lines = 18
local max_height = line_height * max_lines * 1.5
local max_panes = math.floor( max_height / 86 )
...
local function cache( window )
	...
	elseif window.type == "image" then
		size = size + window.tam
...
local function draw( window, y )
	...
	elseif window.type == "button" then
		...
		if not window.button then
			window.button = dxCreateButton (x + width / 4, y + 3, width / 2, line_height - 4, tostring( window.text ), false, true)
			addEventHandler( "onClientElementDestroy", window.button, function( ) window.button = nil end, false )
			if window.onClick then
				addEventHandler( "ondxClientClick", window.button,
			...
			local bx, by = getDXPosition( window.button )
			if by ~= y + 3 then
				setDXPosition(window.button, bx, y + 3)
			end
		end
		y = y + line_height
	elseif window.type == "image" then
		...
		--if not window.button then
			...
			--window.button = dxCreateButton (x + width / 4, y + 3, width / 2, line_height - 4, tostring( window.text ), false, false)
		dxDrawImage( x + width / 4, y + 3, width / 2, line_height*2.2 - 4, window.image, 0, 0, 0, tocolor( 255, 255, 255, 255 ), true )		
		y = y + line_height*2.2
	elseif window.type == "pane" then
...
function show( name, forced, dontEnableInput, mouse, ignored )
	if getElementData(getLocalPlayer(), "openingGUI") == true then return end
	setElementData(getLocalPlayer(), "openingGUI", true)
	setTimer(setElementData, 500, 1, getLocalPlayer(), "openingGUI", false)
	if window then
		hide( )
	end
	if getElementData(getLocalPlayer(), "nogui") == true and not ignored then return end
...
function hide()
	if getElementData(getLocalPlayer(), "inTienda") then setTimer(setElementData, 1000, 1, getLocalPlayer(), "inTienda", nil) end
	if window then
